...
with col_f1:
    status_filter = st.selectbox("Status", ["alle"] + sorted(df["status"].unique().tolist()))
with col_f2:
    fit_filter = st.selectbox("Strategischer Fit", ["alle"] + sorted(df["strategischer_fit"].unique().tolist()))
with col_f3:
...
st.bar_chart(gefiltert.set_index("feature")[["score"]], height=350)

st.markdown("---")
st.subheader("Detail-Ansicht")
ausgewaehlt = st.selectbox("Feature auswählen", options=gefiltert["feature"].tolist(), index=0)
...
col3.metric("Fit", detail["strategischer_fit"])
